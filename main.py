...
class PaintApp:
    def __init__(self, root):
        self.root = root
        self.root.title("KatPaint")
        self.canvas = tk.Canvas(self.root, bg="white")

        self.root.attributes("-fullscreen", True)
        title_frame = tk.Frame(self.root, bg="#94c2ff", height=30)
        title_frame.pack(fill="x", side=tk.TOP)
        window_title = tk.Label(title_frame, text="KatPaint", bg="#94c2ff", fg="black", font=("Arial", 12))
    ...
    def setup_toolbar(self):
        ...
        brush_size_slider = tk.Scale(self.toolbar, from_=1, to=30, orient=tk.HORIZONTAL, label="Размер кисти", command=self.set_brush_size)
        brush_size_slider.set(2)
        brush_size_slider.pack(side=tk.LEFT, padx=5)

        shapes_label = tk.Label(self.toolbar, text="Фигуры:")
        shapes_label.pack(side=tk.LEFT)

        self.shapes_combobox = ttk.Combobox(self.toolbar, values=["Кисть", "Линия", "Прямоугольник", "Овал"], state="readonly")
        self.shapes_combobox.current(0)
        self.shapes_combobox.pack(side=tk.LEFT)

        draw_button = tk.Button(self.toolbar, text="Выбрать", command=self.paint_shape)
